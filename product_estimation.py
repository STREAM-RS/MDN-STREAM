...
def get_estimates(args, x_train=None, y_train=None, x_test=None, y_test=None, output_slices=None, dataset_labels=None,
                  x_sim=None, y_sim=None, return_model=False, return_coefs=False):
    '''
...
def main(kwargs):
    ...
    if args.filename:
        ...
        if filename.is_file():
            ...
            x_test, *_ = _load_datasets(['Rrs'], [filename], bands)
    ...
    # Train a model with partial data, and benchmark on remaining
        ...
        labels = get_labels(bands, slices, y_test.shape[1], wavelengths_ad_ag=get_sensor_bands(f'{args.sensor}-adag',
                                                                                               args) if args.use_HICO_aph else None,
                            wavelengths_aph=get_sensor_bands(f'{args.sensor}-aph', args) if args.use_HICO_aph else None,
                            use_HICO_aph=args.use_HICO_aph)
        ...
        for product in products:
            ...
            if product in ['aph'] and True:
                plot_spectra(y_test[:, slices[product]], benchmarks, bands, labels[slices[product]], product,
                             args.sensor, args=args, y_full=y_test, slices=slices)
