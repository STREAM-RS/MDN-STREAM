...
class MDN:
    ...
    def fit(self, X, Y, output_slices=None, **kwargs):
        with get_device(self.config):
            checkpoint_legacy = self.model_path.joinpath('checkpoint')
            checkpoint = self.model_path.joinpath('trained_model.h5')

            if (checkpoint.exists() or checkpoint_legacy.exists()) and not self.no_load:
    ...
    def loss(self, y, output):
        ...
        likelihood = tf.cond(tf.reduce_any(tf.math.is_nan(y)), lambda: impute(mix, y, self.imputations),
                             lambda: mix.log_prob(y))
    ...
    def load(self):
        ...
        if self.model_path.joinpath('config.json').is_file():
    ...
    def _get_threshold_estimate(self, coefs, threshold=0.5):
        ...
        return tf.compat.v2.where(
            tf.expand_dims(tf.math.greater(tf.reduce_max(prior, 1) / threshold, tf.math.sign(threshold)), -1),
            top_estimate, avg_estimate)
