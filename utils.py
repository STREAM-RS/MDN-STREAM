...
def download_example_imagery(sensor, date, location, dest=None):
    ...
    if dest == None:
        ...
        assert os.path.isdir(dest), f"present valid location"
        dest = dest + f"/data/example_imagery/{sensor}/{date}/{location}/sat_cube.nc"
            ...
            for feature in features:
                if feature not in data:
                    if feature in ['Rrs', 'rhos', 'rayleigh_corrected']:
                        bands, band_data = _get_tile_wavelengths(nc_data, feature, sensor, allow_neg, landmask=landmask,
                                                                 args=args)
...
def generate_config(args, create=True, verbose=True):
    ...
    try:
        ...
        raise Exception(
            'Weights have not yet been downloaded - please run "git lfs install" and then "git lfs pull" inside the repository folder')
...
def _load_datasets(keys, locs, wavelengths, allow_missing=False):
    ...
    def loadtxt(name, loc, required_wvl):
        ...
        try:
            ...
            data = np.loadtxt(dloc, delimiter=',',
                              dtype=float if name not in ['../Dataset', '../meta', '../datetime'] else str,
                              comments=None)
    ...
    def get_valid(name, loc, required_wvl, margin=2):
        ...
        if not all([w1 == w2 for w1, w2 in zip(wvls[valid], required_wvl)]):
            ...
            assert (len(np.unique(valid)) == len(valid) == len(required_wvl)), [valid, wvls[valid].flatten(),
                                                                                required_wvl]
        return valid
    ...
    for loc in locs:
        try:
            ...
            print(
                f'\tN={len(loc_data[0]):>5} | {loc.parts[-1]} / {loc.parts[-2]} ({[np.isfinite(ld).all(1).sum() if ld.dtype.type is not np.str_ else len(ld) for ld in loc_data[1:]]})')
    ...
    assert (all([x.shape[1] == x_data[0].shape[1] for x in
                 x_data])), f'Differing number of {keys[0]} wavelengths: {[x.shape for x in x_data]}'
    ...
    for i, key in enumerate(keys[1:]):
        ...
        assert (all([y[i].shape[1] == y_data[0][i].shape[1] for y in
                     y_data])), f'{key} shape mismatch: {[y.shape for y in y_data]}'
    ...
    for product in ['ad', 'ag']:
        if product in slices:
            ...
            for i, sample in enumerate(y_data):
                ...
                assert (
                        len(sample) > 5), f'Number of bands should be larger, when fitting exponential: {product}, {sample.shape}'
                assert (len(sample) == len(
                    wavelengths)), f'Sample size / wavelengths mismatch: {len(sample)} vs {len(wavelengths)}'
                ...
                if np.all(np.isfinite(sample)) and np.min(sample) > -0.1:
                    try:
                        ...
                        if mdsa(sample[None, :], new_sample[None, :]) < 10:
                            ...
                            remove[i] = True  # Exponential could be fit, but error was too high
                    except:
                        remove[
...
def get_data(args):
    ...
    x_data, y_data, slices, sources = _load_datasets(data_keys, sensor_loc, bands,
                                                     allow_missing=('-nan' in args.sensor) or (
                                                             getattr(args, 'align', None) is not None))
    ...
    if getattr(args, 'align', None) is not None:
        ...
        x_align, y_align, slices_align, sources_align = map(list,
                                                            zip(*[_load_datasets(data_keys, loc,
                                                                                 get_sensor_bands(a, args),
                                                                                 allow_missing=True) for a, loc in
                                                                  zip(align, align_loc)]))
    ...
    if '-nan' not in args.sensor:
        (x_data, *_), (y_data, *_), (sources, *_) = _filter_invalid(x_data, y_data, slices, other=[sources],
                                                                    allow_nan_out=not using_feature(args,
                                                                                                    'sim') and len(
                                                                        data_keys) > 2)
...
def _fix_tchl(y_data, sources, slices, data_path, debug=False):
    ...
    lonlats = meta[['east_longitude', 'west_longitude', 'north_latitude', 'south_latitude']].apply(
        lambda v: v.apply(lambda v2: v2.split('||')[0]))
    ...
    lonlats = lonlats.apply(
        lambda v: pd.to_numeric(v.apply(lambda v2: v2.split('::')[1].replace('[deg]', '')), 'coerce'))
    ...
    # coef = [-0.12, 0.9, 0.001]
    y_data[fix, slices['chl']] = np.sum(np.array(coef) * y_data[fix, slices['chl']] ** np.arange(len(coef)), 1,
                                        keepdims=False)
...
def write_cube_to_netcdf4(data, save_name="user_generated.nc", lat=None, lon=None, band_names=None):
    """
    This function can be used to save 3D cube as netcdf with the apporpriate latitude and longitude information

    :param data: [np.ndarray: nRows X nCols X nBands]
    This the 3D cube that is to be saved as a netCDF4 file. In this implementation it must be a 3D numpy array

    :param save_name: [string] (Default: "user_generated.nc")
    The file name and location where the netCDF4 is to be saved

    :param lat: [np.ndarray: nRows X nCols] (Default: None)
    The pixel by pixel latitude information to be saved with the netCDF4. Not included by default. If given must have
    the same number of rows and columns as the <data> cube

    :param lon: [np.ndarray: nRows X nCols] (Default: None)
    The pixel by pixel longitude information to be saved with the netCDF4. Not included by default. If given must have
    the same number of rows and columns as the <data> cube

    :param band_names: [list:str] (Default: None)
    The name to be assiged to each band. If nothing is provided (default), band are sequentially as "Band-1", "Band-2"
    and so on
    ...
    assert isinstance(data, np.ndarray) and (len(data.shape) == 3), "The variable <data> must be a 3D numpy array"
    assert isinstance(save_name, str), "The variable <save_name> must be a string"
    try:
        Path(save_name).resolve()
        exist = True
    except (OSError, RuntimeError):
        exist = False
    assert exist, "Not a valid file path"

    if lat is not None:
        assert lon is not None, "If providing <lat> must also provide <lon>"
        assert isinstance(lat, np.ndarray) and (len(lat.shape) == 2), "The variable <lat> must be a 2D numpy array"
        assert lat.shape == lon.shape and (data.shape[:-1] == lat.shape), "<lat> and <lon> must have the same number of" \
                                                                          "rows and columns as <data>"
        assert np.nanmin(lat) >= -180 and np.nanmax(lat) <= 180, "Latitude values must be between -90 and 90"
        assert np.nanmin(lon) >= -180 and np.nanmin(lon) <= 180, "Longitude values must be between -180 and 180"

    if band_names is not None:
        assert all(isinstance(item, str) for item in band_names), "All band names must be stings"
        assert len(band_names) == data.shape[2], f"Names provided for {len(band_names)} bands but data has" \
                                                 f" {data.shape[2]} bands"
    else:
        band_names = [f"Band-{item}" for item in range(data.shape[2])]

    'Create a multi-dimensional x-array'
    ...
    ds = ds.rename({i: name for i, name in enumerate(band_names)})

    'Add Encoding to save space'
    comp = dict(zlib=True, complevel=4)
    encoding = {var: comp for var in ds.data_vars}

    'save the data'
    ds.to_netcdf(save_name, encoding=encoding)
