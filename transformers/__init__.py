...
def generate_scalers(args, x_train=None, x_test=None, column_bagging=False):
    ...
    many_features = column_bagging and any(
        x is not None and (x.shape[1] - len(wavelengths)) > 15 for x in [x_train, x_test])
    ...
    if column_bagging and using_feature(args, 'bagging') and (using_feature(args, 'ratio') or many_features):
        n_extra = 0 if not using_feature(args, 'ratio') else RatioTransformer(
            wavelengths).get_n_features()  # Number of ratio features added
        args.x_scalers = [
                             serialize(BaggingColumnTransformer, [len(wavelengths)],
                                       {'n_extra': n_extra, 'seed': args.seed}),
                         ] + args.x_scalers
