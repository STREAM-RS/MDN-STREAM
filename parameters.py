...
parser.add_argument("--data_loc", default="/home/ryanoshea/in_situ_database/Working_in_situ_dataset/in_situ_data",
                    help="Location of in situ data")
parser.add_argument("--sim_loc", default="D:/Data/Simulated", help="Location of simulated data")
parser.add_argument("--n_redraws", default=50, type=int,
                    help="Number of plot redraws during training (i.e. updates plot every n_iter / n_redraws iterations); only used with --plot_loss.")
parser.add_argument("--n_rounds", default=10, type=int,
                    help="Number of models to fit, with median output as the final estimate")

''' Flags '''
parser.add_argument("--threshold", default=None, type=float,
                    help="Output the maximum prior estimate when the prior is above this threshold, and the weighted average estimate otherwise. Set to None, thresholding is not used")
parser.add_argument("--avg_est", action="store_true",
                    help="Use the prior probability weighted mean as the estimate; otherwise, use maximum prior")
...
parser.add_argument("--save_stats", action="store_true",
                    help="Store partial training statistics & estimates for later analysis")
...
update = parser.add_argument_group('Model Parameters',
                                   'Parameters which require a new model to be trained if they are changed')
update.add_argument("--sat_bands", default=False, action="store_true",
                    help="Use bands specific to certain products when utilizing satellite retrieved spectra")
update.add_argument("--benchmark", default=False, action="store_true",
                    help="Train only on partial dataset, and use remaining to benchmark")
...
update.add_argument("--align", default=None,
                    help="Comma-separated list of sensors to align data with; passing \"all\" uses all sensors (See meta.py for available options)")
...
# update.add_argument("--filter_ad_ag",     default=False,           help="Filters ad/ag based on fit to exponential")
...
flags.add_argument("--use_boosting", action="store_true",
                   help="Use boosting when training in multiple trials (no longer implemented)")
...
flags.add_argument("--use_excl_Rrs", action="store_true",
                   help="Drop raw Rrs features from the input when using ratio features")
flags.add_argument("--use_all_ratio", action="store_true",
                   help="Use exhaustive list of ratio features instead of only those found in literature (should be combined with --use_kbest)")

flags.add_argument("--use_kbest", type=int, nargs='?', const=5, default=0,
                   help="Select top K features to use as input, based on mutual information")

''' Hyperparameters '''
hypers = parser.add_argument_group('Hyperparameters',
                                   'Hyperparameters used in training the model (also requires model retrain if changed)')
...
hypers.add_argument("--imputations", default=5, type=int,
                    help="Number of samples used for imputation when handling NaNs in the target")
