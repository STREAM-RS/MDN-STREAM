...
def _create_metric(metric, y_true, y_est, longest=None, label=None):
    ...
    prec = (
        1 if abs(metric(y_true, y_est)) < 100 and metric.__name__ not in ['N'] else 0) if ispct or metric.__name__ in [
        'N'] else 3
...
def _create_stats(y_true, y_est, metrics, title=None):
    ...
    longest = max(
        [len(metric.__name__.replace('SSPB', 'Bias').replace('MdSA', 'Error').replace('^', '')) for metric in metrics])
...
def _create_multi_feature_stats(y_true, y_est, metrics, labels=None):
    ...
    assert (len(labels) == y_true.shape[1] == y_est.shape[
        1]), f'Number of labels does not match number of features: {labels} - {y_true.shape}'

    title = metrics[0].__name__.replace('SSPB', 'Bias').replace('MdSA', 'Error')
    longest = max([len(label.replace('^', '')) for label in labels])
    statbox = [_create_metric(metrics[0], y1, y2, longest=longest, label=lbl) for y1, y2, lbl in
               zip(y_true.T, y_est.T, labels)]
    statbox = [rf'$\mathbf{{\underline{{{title}}}}}$'] + statbox
    return statbox


def add_stats_box(ax, y_true, y_est, metrics=[mdsa, sspb, slope], bottom_right=False, bottom=False, right=False,
                  x=0.025, y=0.97, fontsize=16, label=None):
...
def draw_map(*lonlats, scale=0.2, world=False, us=True, eu=False, labels=[], ax=None, gray=False, res='i',
             **scatter_kws):
    ...
    kwargs.update(
        dict(zip(['llcrnrlat', 'urcrnrlat', 'llcrnrlon', 'urcrnrlon'], MAP['lcc' if 'lcc' in MAP else 'cyl'])))
    ...
    if not world:
        m.readshapefile(Path(__file__).parent.joinpath('map_files', 'st99_d00').as_posix(), name='states',
                        drawbounds=True, color='k', linewidth=0.5, zorder=11)
    ...
    if labels:
        colors = ['aqua', 'orangered', 'xkcd:tangerine', 'xkcd:fresh green', 'xkcd:clay', 'magenta', 'xkcd:sky blue',
                  'xkcd:greyish blue', 'xkcd:goldenrod', ]
        markers = ['o', '^', 's', '*', 'v', 'X', '.', 'x', ]
        mod_cr = False
    ...
    if world:
        ...
        if us:
            ...
            ax_ins = inset_axes(ax, width=PLOT_WIDTH * size, height=PLOT_HEIGHT * size, loc='center',
                                bbox_to_anchor=loc, bbox_transform=ax.transAxes, axes_kwargs={'zorder': 5})
        ...
        if eu:
            ax_ins = inset_axes(ax, width=PLOT_WIDTH * size, height=PLOT_HEIGHT * size, loc='center',
                                bbox_to_anchor=(0.75, -0.05), bbox_transform=ax.transAxes, axes_kwargs={'zorder': 5})
...
def plot_scatter(y_test, benchmarks, bands, labels, products, sensor, title=None, methods=None, n_col=3, img_outlbl='',
    ...
    if len(labels) > 3:
        product_bands = {
            'default': [443, 482, 561, 655] if 'OLI' in sensor else [443, 490, 560, 620, 673,
                                                                     710] if 'OLCI' in sensor else [444, 530, 625,
                                                                                                    673, ] if args.use_HICO_aph else [
                409, 443, 478, 535, 620, 673, 690, 724]
            # [415,443,490,501,550,620,673,690] #[443,482,561,655]
        ...
        target = [closest_wavelength(w, bands if not args.use_HICO_aph else get_sensor_bands(f'{sensor}-aph', args)) for
                  w in product_bands.get(products[0], product_bands['default'])]
        print('Target', target)
        ...
        if args.use_HICO_aph and (products[0] == 'ad' or products[0] == 'ag'):
            product_bands = {
                'default': [get_sensor_bands(f'{sensor}-adag', args)[1], get_sensor_bands(f'{sensor}-adag', args)[4],
                            get_sensor_bands(f'{sensor}-adag', args)[6]]
                # list(get_sensor_bands('HICO-adag', args)) ##[415,444,478,535,564,593,621]#list(get_sensor_bands('HICO-adag', args))
            }
            target = [
                closest_wavelength(w, bands if not args.use_HICO_aph else get_sensor_bands(f'{sensor}-adag', args)) for
                w in product_bands.get(products[0], product_bands['default'])]
        ...
        if plot_order is None:
            ...
            if 'chl' in products and len(products) == 1:
                ...
                plot_order = ['MDN', 'Gilerson_2band', 'Smith_Blend'] if 'OLI' not in sensor else [
                    'MDN', ]  # OC3 GIOP , 'Gilerson_2band', 'Smith_Blend'
            ...
            if 'pc' in products and len(products) == 1:
                #     # benchmarks = benchmarks['chl']
                #     if 'MLP' in benchmarks:
                plot_order = ['MDN', 'Schalles', 'Sim2005'] if 'OLI' not in sensor else [
                    'MDN', ]  # 'Schalles','Sim2005'
            if len(products) == 1:
                plot_order = [val for i, val in enumerate(plot_order) if val in benchmarks[products[0]].keys()]
            #     else:
            #         plot_order = ['MDN']
    ...
    fig_size = 5
    n_col = max(n_col, sum(plot_label_aph if args.use_HICO_aph and (
            products[0] == 'ad' or products[0] == 'ag') else plot_label))
    n_row = max(1,
                int(not plot_bands) + int(0.5 + len(plot_order) / (1 if plot_bands else n_col)) - int(not plot_bands))
    ...
    colors = ['xkcd:sky blue', 'xkcd:tangerine', 'xkcd:fresh green', 'xkcd:greyish blue', 'xkcd:goldenrod', 'xkcd:clay',
              'xkcd:bluish purple', 'xkcd:reddish']
        ...
        for est_idx, est_lbl in enumerate(plot_order[product] if isinstance(plot_order, dict) else plot_order):
            ...
            if est_lbl == 'GIOP' or est_lbl == 'QAA':
                ...
                for required_wvl_i in required_wvl:
                    ...
                    valid_bool.append([wvl_diff_bool_i if np.argmin(wvl_diff) == i else False for i, wvl_diff_bool_i in
                                       enumerate(wvl_diff_bool)])

                valid_wvl_ad_ag = np.any(np.array(valid_bool), axis=0)

                if not args.use_HICO_aph or (
                        np.shape(benchmarks[product][est_lbl])[1] != np.shape(get_sensor_bands(f'{sensor}-aph', args))[
                    0] and args.use_HICO_aph and product == 'aph') or (
                        np.shape(benchmarks[product][est_lbl])[1] != np.shape(get_sensor_bands(f'{sensor}-adag', args))[
                    0] and args.use_HICO_aph and (product == 'ad' or product == 'ag')):
                    benchmarks[product][est_lbl] = np.reshape(benchmarks[product][est_lbl], (-1, np.shape(
                        get_sensor_bands(sensor, args))[
                        0]))  # if not args.use_HICO_aph  else np.reshape(benchmarks[product][est_lbl],(-1,np.shape(get_sensor_bands('HICO-aph', args))[0])) #if np.shape(benchmarks[product][est_lbl])[0] == 1786*np.shape(get_sensor_bands(sensor, args))[0] else  np.reshape(benchmarks[product][est_lbl],(1786,np.shape(get_sensor_bands(sensor, args))[0])) #np.shape(benchmarks[product]['MDN']))
                # Resample to HICO_aph wavelengths
                if args.use_HICO_aph and np.shape(benchmarks[product][est_lbl])[1] == \
                        np.shape(get_sensor_bands(sensor, args))[0] and product == 'aph':
                    benchmarks[product][est_lbl] = benchmarks[product][est_lbl][:,
                                                   [i in get_sensor_bands(f'{sensor}-aph', args) for i in
                                                    get_sensor_bands(sensor, args)]]
                if args.use_HICO_aph and np.shape(benchmarks[product][est_lbl])[1] == \
                        np.shape(get_sensor_bands(sensor, args))[0] and (product == 'ad' or product == 'ag'):
                    benchmarks[product][est_lbl] = benchmarks[product][est_lbl][:,
                                                   valid_wvl_ad_ag]  # [ i in get_sensor_bands(f'{sensor}-adag', args) for i in  get_sensor_bands(sensor, args)]]

            y_est = benchmarks[product][est_lbl] if isinstance(plot_order, dict) else benchmarks[product][est_lbl][
                ..., plt_idx]
            ax = axes[curr_idx]
            ...
            l_kws = {'color': color, 'path_effects': [pe.Stroke(linewidth=4, foreground='k'), pe.Normal()],
                     'zorder': 22, 'lw': 1}
            ...
            if est_lbl == 'MDN':
                ...
                est_lbl = est_lbl.replace('Mishra_', '').replace('Gons_2band', 'Gons').replace('Gilerson_2band',
                                                                                               'GI2B').replace('Smith_',
                                                                                                               '').replace(
                    'Cao_XGB',
                    'BST')  # .replace('Schalles', 'Schalles and Yacobi (2000)').replace('Sim2005', 'Simis et al. 2005')#.replace('Cao_', 'Cao\ ')
            ...
            if product not in ['chl', 'tss', 'cdom', 'pc', 'nap', 'SCDOM', 'SNAP'] and last_col:
                ...
                if args.use_HICO_aph and product == 'aph':
                    ...
                    ax2.set_ylabel(fr'$\mathbf{{{bands_aph[plt_idx]:.0f}nm}}$' f"\n" f"N={sum(np.isfinite(y_true))}",
                                   fontsize=20)
                else:
                    if args.use_HICO_aph and (product == 'ad' or product == 'ag'):
                        bands_adag = get_sensor_bands(f'{sensor}-adag', args)
                        ax2.set_ylabel(
                            fr'$\mathbf{{{bands_adag[plt_idx]:.0f}nm}}$' f"\n" f"N={sum(np.isfinite(y_true))}",
                            fontsize=20)
                    else:
                        ax2.set_ylabel(fr'$\mathbf{{{bands[plt_idx]:.0f}nm}}$' f"\n" f"N={sum(np.isfinite(y_true))}",
                                       fontsize=20)

            minv = -3 if product == 'cdom' else 0.000 if product == 'Scdom443' else .005 if product == 'Snap443' else int(
                np.nanmin(y_true_log)) - 1 if product != 'aph' and product != 'ag' and product != 'ad' else -4
            maxv = 3 if product == 'tss' else 4 if product == 'chl' else .035 if product == 'Scdom443' else .02 if product == 'Snap443' else int(
                np.nanmax(y_true_log)) + 1 if product != 'aph' and product != 'ag' and product != 'ad' else 2
            ...
            if valid.sum():
                sns.regplot(y_true_log[valid], y_est_log[valid], ax=ax, scatter_kws=s_kws, line_kws=l_kws, fit_reg=True,
                            truncate=False, robust=True, ci=None)
                kde = sns.kdeplot(y_true_log[valid], y_est_log[valid], shade=False, ax=ax, bw='scott', n_levels=4,
                                  legend=False, gridsize=100, color='k')
                # kde.collections[2].set_alpha(0)

            invalid = np.logical_and(np.isfinite(y_true_log), ~np.isfinite(y_est_log))
            if invalid.sum():
                ax.scatter(y_true_log[invalid], [minv] * (invalid).sum(), color='r', alpha=0.4,
                           label=r'$\mathbf{%s\ invalid}$' % (invalid).sum())
                ax.legend(loc='lower right', prop={'weight': 'bold', 'size': 16})

            add_identity(ax, ls='--', color='k', zorder=20)

            if valid.sum():
                add_stats_box(ax, y_true[valid], y_est[valid], metrics=[mdsa, sspb, slope])
                ...
                # else:
                if plot_order[est_idx] not in args.summary_stats.keys(): args.summary_stats[plot_order[est_idx]] = {}
                args.summary_stats[plot_order[est_idx]][label[1]] = create_stats_HPC(y_true[valid], y_est[valid],
            ...
            if first_row or not plot_bands or (isinstance(plot_order, dict) and plot_order[product][est_idx] != 'MDN'):
                if est_lbl == 'BST':
                    ...
                    # ax.set_title(fr'$\mathbf{{\large{{{est_lbl}}}}}$'+'\n'+r'$\small{\textit{(Cao\ et\ al.\ 2020)}}$', fontsize=18)
                    ax.set_title(fr'$\mathbf{{\large{{{est_lbl}}}}}$' + r'$\small{\textit{\ (et\ al.\ 2011)}}$',
                                 fontsize=18, linespacing=0.95)


                elif est_lbl == 'Mannino':
                    # ax.set_title(fr'$\mathbf{{\large{{{est_lbl}}}}}$'+'\n'+r'$\small{\textit{(Cao\ et\ al.\ 2020)}}$', fontsize=18)
                    ax.set_title(fr'$\mathbf{{\large{{{est_lbl}}}}}$' + r'$\small{\textit{\ (et\ al.\ 2008)}}$',
                                 fontsize=18, linespacing=0.95)

                elif est_lbl == 'Novoa':
                    # ax.set_title(fr'$\mathbf{{\large{{{est_lbl}}}}}$'+'\n'+r'$\small{\textit{(Cao\ et\ al.\ 2020)}}$', fontsize=18)
                    ax.set_title(fr'$\mathbf{{\large{{{est_lbl}}}}}$' + r'$\small{\textit{\ (et\ al.\ 2017)}}$',
                                 fontsize=18, linespacing=0.95)
                    ...
                    # ax.set_title(fr'$\mathbf{{\large{{{est_lbl}}}}}$'+'\n'+r'$\small{\textit{(Cao\ et\ al.\ 2020)}}$', fontsize=18)
                    est_lb = 'Gilerson'
                    ax.set_title(fr'$\mathbf{{\large{{{est_lb}}}}}$' + r'$\small{\textit{\ (et\ al.\ 2010)}}$',
                                 fontsize=18, linespacing=0.95)
                elif est_lbl == 'Blend':
                    # ax.set_title(fr'$\mathbf{{\large{{{est_lbl}}}}}$'+'\n'+r'$\small{\textit{(Cao\ et\ al.\ 2020)}}$', fontsize=18)
                    est_lb = 'Smith'
                    ax.set_title(fr'$\mathbf{{\large{{{est_lb}}}}}$' + r'$\small{\textit{\ (et\ al.\ 2018)}}$',
                                 fontsize=18, linespacing=0.95)
                elif est_lbl == 'Nechad':
                    # ax.set_title(fr'$\mathbf{{\large{{{est_lbl}}}}}$'+'\n'+r'$\small{\textit{(Cao\ et\ al.\ 2020)}}$', fontsize=18)
                    # est_lb='Smith'
                    ax.set_title(fr'$\mathbf{{\large{{{est_lbl}}}}}$' + r'$\small{\textit{\ (et\ al.\ 2010)}}$',
                                 fontsize=18, linespacing=0.95)
                    ...
                    ax.set_title(fr'$\mathbf{{\large{{{est_lbl}}}}}$' '\n' f'(N={sum(np.isfinite(y_true))})',
                                 fontsize=18) if first_col else ax.set_title(fr'$\mathbf{{\large{{{est_lbl}}}}}$',
                                                                             fontsize=18)
            # ax.x
...
def plot_spectra(y_test, benchmarks, bands, labels, products, sensor, title=None, methods=None, n_col=3, img_outlbl='',
                 args=None, y_full=None, slices=None):
    ...
    for index in range(len(y_test)):
        if sum(np.isnan(y_test[index, :])) or sum(np.isnan(y_full[:, slices['chl']][index, :])) or sum(
                np.isnan(y_full[:, slices['pc']][index, :])): continue
        ...
        filename = folder.joinpath(
            f'{out_dir}/spectral_products/{img_outlbl}Spectral_products_{sensor}_{index}_test.png')
        plt.tight_layout()
        # plt.subplots_adjust(wspace=0.35)
